...
async def send_invitation(
    ...
    await service.send_invitation(
        company_id=company_id, invited_user_id=user_id, owner_id=current_user.id
    )
    return {"detail": "Invitation sent successfully."}


@router.delete("/invitations/{user_id}", status_code=status.HTTP_204_NO_CONTENT)
async def cancel_invitation(company_id: UUID, user_id: UUID, service: ActionService):
    await service.cancel_invitation(company_id=company_id, canceled_user_id=user_id)


@router.post("/requests/{user_id}/accept", status_code=status.HTTP_200_OK)
async def accept_request(company_id: UUID, user_id: UUID, service: ActionService):
    await service.accept_request(company_id=company_id, requester_id=user_id)
    return {"detail": "Request accepted. User is now a member."}


@router.post("/requests/{user_id}/decline", status_code=status.HTTP_200_OK)
async def decline_request(company_id: UUID, user_id: UUID, service: ActionService):
    await service.decline_request(company_id=company_id, requester_id=user_id)
